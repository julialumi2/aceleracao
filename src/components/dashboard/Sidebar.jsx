...
const NAV_ITEMS = [
  ...
  { key: "restaurante", label: "Dados do Restaurante", icon: Store },
  { key: "cardapio", label: "Ajustes do Cardápio", icon: UtensilsCrossed },
  { key: "contratos", label: "Contratos & Pagamentos", icon: FileSignature },
];

export default function Sidebar({ active, onNavigate, session, onLogout }) {
  return (
    <aside className="hidden w-64 shrink-0 flex-col border-r border-line/60 bg-surface/40 lg:flex">
      <div className="flex items-center gap-2 px-6 py-6">
        ...
        <span className="font-display text-base tracking-wide text-ink">Aceleração Delivery</span>
      </div>
      ...
      <div className="border-t border-line/60 p-4">
        <div className="flex items-center gap-3 rounded-xl px-2 py-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-medium text-ink">{session?.name || "Usuário"}</p>
